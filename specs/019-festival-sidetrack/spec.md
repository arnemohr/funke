...
- `GET .../headcount` — per slot `{total, by_tier, cap, overbooked}` incl. group sizes + overall accommodation totals **split into angefragt vs. zugesagt** (Zelt/Camper counts incl. group sizes, each as `{requested, approved}` — Ä15/Ä17; the approved numbers are the real pitch demand) (one `list_registrations` pass, like `get_registration_stats`, `registration_service.py:738`)
